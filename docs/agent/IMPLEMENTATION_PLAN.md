...
上記 D-01〜D-06、旧 `links` DDL、永続化・復元責任者のいずれかが未確定の場合、アプリ基盤の実装は進めてもデータ移行および本番切替は実施しない。
...
1. `export-postgres` を読み取り専用接続・dry-run・JSON Lines 出力で実装する。旧 `links` と `access_logs` の件数・実行情報を記録する。
2. `migrate-uploads` を実装し、シンボリックリンクを拒否して画像/APK を再検証・SHA-256 照合・UUID 名へのコピー・対応表出力を行う。
3. `import-sqlite` をトランザクション、タグ統合、アクセスログ保持期限、ID 維持、再実行可能性、例外一覧の出力を備えたものにする。
4. `verify-migration` を実装し、リンク、タグ集合、保持対象ログ、ファイル数/サイズ/hash、集計、人気上位 10 件、DB integrity を照合する。
5. DB と uploads を同一世代 ID で保存するバックアップ・復元・integrity check を実装する。各スクリプトは dry-run、機械可読 JSON、秘密値非出力、明確な終了コードを備える。
6. 本番構成では `/data/portal.db` と `/data/uploads/{images,apk}` を同一の永続ボリュームにマウントし、起動時に migration を適用して静的 Web ファイルを配信する。

**テスト:** 移行の件数/ID/タグ/hash 照合、無効 URL の例外化、保持期限外ログの除外、バックアップから隔離環境への復元、コンテナ再作成後の永続性。

### Phase 6: ステージング受入と本番切替
...
Phase 4 は Phase 2 の公開/管理 API が安定してから開始する。Phase 5 のツールは Phase 2 と Phase 3 のデータモデル・保存形式の確定後に実装し、Phase 6 はすべての自動テストおよび移行照合が成功してから開始する。
...
- 旧 PostgreSQL/ファイルからの検証移行で、承認済み例外以外の差分がない。
